...
- Verified: @keicoin/work is published at 0.4.2 and resolves from the kei-transaction@0.8.0 umbrella, per the npm registry; the site Worker's one route is the same-origin work endpoint, per keicoin-site/README.md.
...
Every package is on npm and kei-transaction 0.8.0 reaches the coordinated graph: tokens 0.5.3, claims 0.5.1, wallet 0.5.1, economy 0.2.2, market 0.4.0, player-economy 0.1.2, core 0.5.0, and work 0.4.2. Market 0.4.0 adds defensive read bounds and ranks book levels by exact cross-multiplied price ratios; the earlier 0.6.0 umbrella first closed the 0.5.0 gap where kei.shop was undefined and the newer market was unreachable. The umbrella pins caret ranges, so core 0.6.0, claims 0.6.0 and market 0.5.0 are published but out of reach of a plain install until the next umbrella moves the pins.
