...
- Verified: @keicoin/work is published at 0.4.0 and resolves from the kei-transaction@0.6.0 umbrella, per the kei umbrella README.md; the site Worker's one route is the same-origin work endpoint, per keicoin-site/README.md.
...
@keicoin/economy 0.2.0 ships declarative recipes with a dry run before anything is signed, plus weighted loot-table drops bound into the same claim root as an ordinary reward — with the honest caveat that drops are not verifiable randomness, because the roll happens on the game's server. @keicoin/player-economy 0.1.0 ships the player-owned shop that lists, buys, cancels, and gifts through the player's own key.

**Depends on:** [`dl-node-rooted-claims`](#dl-node-rooted-claims), [`dl-node-swap-settlement`](#dl-node-swap-settlement).

**Acceptance**

- Verified: @keicoin/economy is published at 0.2.0 and @keicoin/player-economy at 0.1.0, both reachable from a plain install of kei-transaction@0.6.0, per the kei umbrella README.md.
...
### kei-transaction@0.6.0: the umbrella reaches the whole registry {#dl-sdk-umbrella-release}

<Badge type="tip" text="shipped" /> · repositories `kei-transaction`

Every package is on npm and the umbrella reaches all of it: tokens 0.5.1, claims 0.5.0, wallet 0.4.2, economy 0.2.0, market 0.2.0, player-economy 0.1.0, core and work 0.4.0, under kei-transaction 0.6.0 — the first umbrella release where a plain install matches the registry, closing the 0.5.0 gap where kei.shop was undefined and the newer market was unreachable.

**Acceptance**

- Verified: npm install kei-transaction@0.6.0 in an empty directory resolves all eight @keicoin/* packages, each to exactly one version and fully deduped, verified 4 August 2026, per the kei umbrella README.md.
...
- Verified: @keicoin/wallet is published at 0.4.2 and ships in kei-transaction@0.6.0, per the kei umbrella README.md.
