...
export const SITE = {
  ...
  status:
    'The installable SDK is `kei-transaction@0.6.0`, published 4 August 2026 to close the gap where a plain install resolved an older `@keicoin/market` and no player shop at all — a clean `bun add kei-transaction` now reaches `@keicoin/market@0.2.0`, `@keicoin/player-economy@0.1.0` and loot-table drops in `@keicoin/economy@0.2.0`. Since 3 August 2026 the public testnet settles rooted claims and atomic swaps end to end over `https://testnet.keicoin.org/rpc`. It is one rate-limited, best-effort node with weak consensus, published dev keys and no uptime promise. There is no mainnet. The demos are demos: Carpet Markets rebuilds its no-value mock from a Durable Object event log across eviction; the hosted World of Wonder uses a process-local mock that resets. Create Kei MMO is an unpublished draft; at integration checkpoint `b6edae7`, fresh blank 2D and 3D projects meet criteria 2–6 only at construction scope, including clean-restart server-authored character persistence and a separate player-custodied atomic trade on a private mock chain. Criteria 1, 7, 8, and 9 remain open. A resume token is not account recovery, socket identity is not bound to a Kei wallet, and SQLite WAL does not establish multi-writer or crash-loss guarantees; scale and public hosting, harness-deletion proof, and presentation also remain open.',
} as const
