...
describe('Create Kei MMO documentation boundary', () => {
  ...
  test('states the b6edae7 persistence and economy checkpoint without implying a complete MMO', () => {
    expect(examplesProse).toContain('criteria 2–6 are met only for fresh blank 2D and Babylon.js 3D construction scope')
    expect(examplesProse).toContain('two headless clients observe each other move')
    expect(examplesProse).toContain('`bun run restart-proof`')
    expect(examplesProse).toContain('server-assigned player ID, position, XP, and server-derived level survive')
    expect(examplesProse).toContain('change neither memory nor SQLite')
    expect(examplesProse).toContain('stores only its SHA-256 hash')
    expect(examplesProse).toContain('no Kei economy state')
    ...
    expect(examplesProse).toContain('Criteria 1, 7, 8, and 9 remain open')
    expect(examplesProse).toContain('resume token is not account recovery')
    expect(examplesProse).toContain('socket character is not cryptographically bound to a Kei wallet')
    expect(examplesProse).toContain('not a multi-writer store or a guarantee against crash loss')
    expect(examplesProse).toContain('Scale, public hosting')
