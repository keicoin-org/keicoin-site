...
await kei.claims.pending()` },
...

  {
    ...
    blocks: [
      {
        kind: 'prose',
        text: 'They are ordered by how much they ask of you. **Button** is the SDK in one tab. **Carpet Markets** is what happens when players can issue things and trade them with each other. **World of Wonder** is a real game with the economy replaced, and the one to fork if you are shipping something.',
      },
      {
        kind: 'prose',
...
npm create kei-game my-launchpad -- --template carpet-markets`,
      },
      {
        ...
        links: [
          ...
          {
            href: '/examples/carpet-markets',
            label: 'Carpet Markets — a launchpad with a real order book',
            note: 'Launch a coin and you are holding all of it. Selling is an offer you write and somebody else accepts, in whatever size you choose. Read the transfer policy before you buy: whether a market can exist for a coin at all is a flag the chain enforces, not a promise anybody makes.',
          },
          {
      ...
      {
        kind: 'prose',
        text: 'The pump.fun shape: anybody launches a coin and is minted all of it, and from there it is worth whatever the next person will pay. **This is the worked demo of `@keicoin/market`** — there is no bonding curve and no house, and every trade is an offer one player wrote and another accepted, settled in one block by consensus.',
      },
      {
        kind: 'prose',
        text: 'There is no mechanic here called *rug*. A creator holding the supply can sell it, in whatever size they like, whenever they like — that is not a special power, it is selling, and it is the only thing anybody on the market can do. What the chain decides is whether that market can exist at all, and it decides it once, at issuance:',
      },
      {
        kind: 'table',
        head: ['Issued as', 'What it means'],
        rows: [
          ['`transfer: \'open\'`', 'Anybody can send it to anybody, so there is a real order book — and the creator is holding a million of them.'],
          ['`transfer: \'issuer-only\'`', 'Units move only to or from the issuer. An offer between two holders is an invalid block, so no player-to-player market exists or can.'],
          ['`transfer: \'none\'`', 'Soulbound. It cannot be locked into an offer, so it cannot be sold, by anybody, including whoever made it.'],
        ],
      },
      {
        kind: 'prose',
        text: 'A database can hold the same flag, and a developer can edit the row. That is the entire difference: a player can read the badge, buy the open coin anyway, watch the creator work through their position a thousand at a time, and check afterwards that the chain said so the whole time.',
      },
      {
        kind: 'prose',
        text: 'The price on the page is the settled trades and nothing else. A coin nobody has traded has no price, and it says so rather than quoting a formula — which is what a chain can honestly tell you and an AMM cannot.',
      },
      {
        kind: 'prose',
        text: 'It also puts a number on the issuance burn, in the direction the burn is actually for. Each coin is issued by an account of its own, so a launch pays that account’s **first** burn — one Kei, flat, however many coins are already listed. The escalation in the burn is per account, so it bounds one launcher minting junk rather than taxing whoever arrives last.',
      },
      {
        kind: 'prose',
        text: 'The economy tests assert the badge at the ledger rather than in the app: a soulbound coin cannot be offered at all, an issuer-only coin cannot be traded between two holders, and an open one settles peer-to-peer in whatever size the seller chose.',
      },
