...
export const TRACKS: Track[] = [
  {
    ...
    state:
      'The primitives are shipped and measured. What is being closed is the distance between an economy you can describe in a sentence and the code that runs it. `@keicoin/economy` — declarative recipes with a dry run before anything is signed — **published at 0.1.0 on 4 August 2026** and reachable as `kei.economy` from an install. `@keicoin/player-economy` — the player-owned shop that lists, buys, cancels and gifts through the player\'s own key — is merged on `master` and **not published**, and the published umbrella does not depend on it, so `kei.shop` is absent from an install rather than broken in it. Publishing is a separate approval.',
...

  {
    ...
    blocks: [
      ...
      { kind: 'heading', text: 'What you can install and run today' },
      {
        ...
        head: ['', 'State'],
        rows: [
          ['The SDK', '`bun add kei-transaction` gets you **0.5.0**. The `@keicoin/*` packages under it are no longer on one version line: `tokens` at 0.5.0, `wallet` at 0.4.1, `economy` at 0.1.0, `market` at 0.1.1, and `core`, `claims` and `work` at 0.4.0 — npm resolves that set for you. Wallet, send, receive, issue, mint, transfer, `balanceOf`, `burn`, items and stats, commit, claim, the market, recipes, and the in-game wallet panel ship with TypeScript types. Payment memos are not in the current wire contract; correlate purchases by payment hash.'],
          ['Installable vs merged', 'They are not the same thing, and this row exists because the gap is where a wrong claim gets made. **Installable: `kei-transaction@0.5.0`**, which as of 4 August 2026 includes `@keicoin/economy`, `kei.economy` and `token.burn()` — this row said otherwise until they shipped. **Merged on `master` and still not published:** `@keicoin/player-economy` and `kei.shop`, and `@keicoin/market@0.2.0`’s bounded books, price series, candles and verify-before-signing. Two different reasons they do not reach you: the published umbrella does not depend on player-economy at all, so `kei.shop` is simply `undefined`; and it pins `@keicoin/market@^0.1.1`, a range that stops below `0.2.0`, so publishing the market alone would still not deliver them. **Publishing is a separate approval and has not been given for either.**'],
          ['The market', 'Published as `@keicoin/market@0.1.1`, and **settling on the public testnet**. An offer locks the units at the ledger, a second sale of the same units is refused, and one accept moves both legs — measured against the node, then again over the public URL once the gateway was taught to forward `swap_info` and `account_swaps`. It could be written to and not read from until that landed.'],
  ...
  --plan-only`,
      ...
      { kind: 'heading', text: 'Known and stated plainly' },
      {
        ...
        items: [
          ...
          '**The 0.5.0 SDK release is installable.** Verified against the registry on 4 August 2026, and against a clean `npm install kei-transaction@0.5.0` in an empty directory rather than read off a checkout: `@keicoin/market` resolves at 0.1.1, and `@keicoin/player-economy` does not resolve at all. Create Kei MMO is not released; npm still serves the superseded `create-kei-game@0.2.0` scaffolder, which is a different product.',
