    ...
    page: {
      ...
      blocks: [
        ...
        { kind: 'prose', text: 'The property doing the work is atomicity. A settlement that half-completes is the failure that costs you a weekend and a player\'s trust, and on a chain it is not a state that exists.' },
        {
          ...
          items: [
            '**The market is not merged yet — it is M5.** Offers and atomic settlement (`swap_offer`/`swap_accept`/`swap_cancel`) are in review on both the node and the SDK, not shipped work. See [status](/status).',
            MOCK_CAVEAT,
...
game.onPayment(async ({ from, amount, hash: receiveHash }) => {
  ...
  await reconcile(receive.link)
        ...
        { kind: 'prose', text: 'A purchase is **always** two signed transactions. There is no `charge(someoneElse, …)` and there never will be — a game cannot sign for a player\'s wallet. Any API implying otherwise is a bug.' },
        {
          ...
          items: [
            MOCK_CAVEAT,
            'Free means no transaction fee. Anti-spam is proof-of-work instead — the SDK ships work-server integration for it as of M4 (0.2.0).',
...
await kei.claims.pending()` },
...

  {
    ...
    blocks: [
      { kind: 'prose', text: 'Kei has a **public M3 testnet and merged native M4 claims**. Version 0.2.0 of the SDK packages is published. `Kei.start()` defaults to the best-effort public testnet; the node gate runs the pinned SDK-owned M2 and M4 contracts against a clean native startup.' },
      {
        kind: 'table',
        head: ['', 'State'],
        rows: [
      ...
      { kind: 'heading', text: 'What this means for you' },
      {
        ...
        items: [
          ...
          'Assume the market or the wallets are usable because a pull request for them exists. In review is not merged.',
        ],
      },
      ...
      { kind: 'heading', text: 'Known and stated plainly' },
      {
        ...
        items: [
          ...
          '**Timeline.** M3 is public. Native M4 claims are merged and CI-gated. M5 (market) and M6 (wallets) have open, unmerged pull requests.',
