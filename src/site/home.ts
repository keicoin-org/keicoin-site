...
import { HARNESS_CRITERIA_COUNT, TRACKS, USE_CASES, ISSUER_SNIPPET, PLAYER_SNIPPET } from './content.js'
...
<section><div class="wrap split">
  ...
  <div class="split-evidence">
    <div class="limits">
      ...
      <ul>
        <li><strong>What you can install is <code>kei-transaction@0.6.0</code></strong>, and the <code>@keicoin/*</code> packages npm resolves under it — the first umbrella release where those two lists match. Item stats, the roll-supply fix, <code>token.burn()</code>, <code>kei.economy</code> recipes and loot-table drops, and <code>kei.shop</code> — the player-owned stall, through the player's own key — all install with it.</li>
        ...
        <li><strong>The demos are demos.</strong> <a href="/docs/examples/carpet-markets">Carpet Markets</a> rebuilds its no-value mock from a Durable Object event log across eviction; the hosted <a href="https://mmo.keicoin.org">World of Wonder</a> uses a process-local mock that resets. Carpet Markets is still weaker than the launchpads it is modelled on, and it is not production-ready or mainnet-ready — deliberately, because a launchpad is the worst possible first thing to put on a real network.</li>
        <li><strong>Create Kei MMO does not produce a complete working MMO yet.</strong> It is an unpublished draft: it plans a project and runs one bounded pass at the first step of that plan. At draft integration checkpoint <code>9d1e60a</code>, criteria 2–4 and 6 are met for fresh blank 2D and 3D projects: they install, build, prove two headless clients see each other move, and run one atomic Kei trade held and signed by the players rather than the game server. Criterion 5 and criteria 1, 7, 8, and 9 remain open; a socket character is not yet bound to a Kei wallet.</li>
