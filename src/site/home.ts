...
<section><div class="wrap split">
  ...
  <div class="split-evidence">
    <div class="limits">
      ...
      <ul>
        <li><strong>What you can install is <code>kei-transaction@0.5.0</code></strong>, and the <code>@keicoin/*</code> packages npm resolves under it. Item stats, the roll-supply fix, <code>token.burn()</code> and <code>kei.economy</code> recipes are included. The market it resolves is <code>0.1.1</code>, and <code>kei.shop</code> is <code>undefined</code> — <code>@keicoin/market@0.2.0</code> and <code>@keicoin/player-economy@0.1.0</code> are both published, but this umbrella predates them and does not depend on it.</li>
        ...
        <li><strong>Create Kei MMO does not produce a working game yet.</strong> It is an unpublished draft: it plans a project and runs one bounded pass at the first step of that plan. Of its ${HARNESS_CRITERIA_COUNT} written criteria, only the first partly holds — the result does not install, build, run, put two players in a world together, or clear the presentation gate the ninth criterion sets.</li>
